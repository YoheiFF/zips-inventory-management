// カテゴリメンテ：一覧/検索/登録/編集/削除（インライン編集対応）
(function () {
  document.addEventListener("DOMContentLoaded", () => {
    if (typeof loadHeader === "function") loadHeader("カテゴリメンテナンス画面");
    ...
    async function apiList(keyword = "") {
      console.log("apiList", { keyword });
      const q = keyword ? `?keyword=${encodeURIComponent(keyword)}` : "";
      const res = await fetch(`/api/categories${q}`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      return await res.json(); // [{ category_no, category_name }]
    }
    ...
    async function apiUpdate(no, name) {
      const res = await fetch(`/api/categories/${encodeURIComponent(no)}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ category_name: name })
      });
    ...
    async function apiDelete(no) {
      const res = await fetch(`/api/categories/${no}`, { method: "DELETE" });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      return await res.json();
    }
  ...
  `);
    ...
    $(document).on("click", ".js-save", async function () {
      ...
      if (!newVal) { alert("カテゴリ名は必須です。"); $input.trigger("focus"); return; }
      try {
    ...
    $(document).on("click", ".js-del", async function () {
      const $tr = $(this).closest("tr");
      const id = $tr.data("id");
      if (!confirm(`カテゴリID「${id}」を削除します。よろしいですか？`)) return;
      try {
        await apiDelete(id);
        const list = await apiList($kw.val().trim());
        render(list);
